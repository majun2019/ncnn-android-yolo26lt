...
*训练侧证据见第 5 节图 8 中的 results.png 与验证批次预测图；部署侧证据见同图中的五任务真机视频抽帧组合图。*
...
第一项结果是统一框架形成了从训练侧到设备侧的可见证据链。训练侧可以从图 8(a) 的收敛曲线观察到损失下降与检测指标提升，验证侧可由图 8(b) 的批次预测结果确认 SafeHat 数据集上的类别学习效果，而部署侧则由图 8(c) 的五任务组合真机证据图、表 8 的功能验证项和表 10 的延迟统计共同支撑。这表明，所提出的分层设计不仅能够承载统一调度，也能为论文中的每一类关键结论提供对应证据载体。

**图 8：训练收敛、验证预测与五任务 Android 真机运行证据**
...
*（c）检测、分割、姿态、分类与 OBB 五任务真机视频抽帧组合证据图。统一编号条 01-05 依次对应 Detection、Segmentation、Pose、Classification 与 OBB；前三项位于上排，后两项位于下排。该子图用于证明统一应用内五类任务均已完成真机加载与运行覆盖，其中 OBB 子图主要体现任务切换后界面与推理路径均可正常工作，具体方向解析修复证据见表 8 和表 9。*

SafeHat 案例提供了最完整的部署证据。该检测模型在一个包含十个安全帽和个人防护装备类别的数据集上完成微调，随后导出为 NCNN 并部署到 Android 资产目录。真机运行结果表明，模型能够被正确加载，且输出形状与目标类别数保持一致。设备端阈值调节功能同样有效，这说明部署路径并非静态展示，而是具备交互可控性。与此同时，图 8(c) 进一步补足了检测、分割、姿态、分类与 OBB 五类任务的统一真机运行覆盖证据，使“五任务已在同一 Android 应用内完成切换与执行”的表述不再仅依赖文字和日志支持。
